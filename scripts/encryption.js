// Modified from https://github.com/D0n9X1n/hexo-blog-encrypt
...
// so adding an empty tag and check it when decryption.
...
hexo.extend.filter.register('after_post_render', (data) => {
    ...
    let template = fs.readFileSync(path.resolve(__dirname, `../lib/encryption/hbe.${theme}.html`)).toString();
    ...
    data.content += `<script data-pjax src="${hexo.config.root}js/plugins/hbe.js"></script><link href="${hexo.config.root}css/plugins/hbe.style.css" rel="stylesheet" type="text/css">`;
...
hexo.extend.generator.register('hexo-blog-encrypt', () => [
    {
        'data': () => fs.createReadStream(path.resolve(__dirname, `../lib/encryption/hbe.style.css`)),
        'path': `css/plugins/hbe.style.css`,
    },
    {
        'data': () => fs.createReadStream(path.resolve(__dirname, '../lib/encryption/hbe.js')),
